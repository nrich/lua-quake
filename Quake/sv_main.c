...
void SV_SendServerinfo (client_t *client)
{
	...
	MSG_WriteString (&client->message, GetString(sv.edicts->v.message));
...
// set up the client_t
	...
	// call the progs to get default spawn parms for the new client
		ExecuteProgram (pr_global_struct->SetNewParms);
...
// send over all entities (excpet the client) that touch the pvs
	...
	for (e=1 ; e<sv.num_edicts ; e++, ent = NEXT_EDICT(ent))
	{
		...
		if (ent != clent)	// clent is ALLWAYS sent
		{
			...
			if (!ent->v.modelindex || !GetString(ent->v.model)[0])
				continue;
...
//	if (ent->v.weapon)
	...
	if (sv.protocol != PROTOCOL_NETQUAKE)
	{
		if (bits & SU_WEAPON && SV_ModelIndex(GetString(ent->v.weaponmodel)) & 0xFF00) bits |= SU_WEAPON2;
...
// [always sent]	if (bits & SU_ITEMS)
	...
	if (bits & SU_WEAPON)
		MSG_WriteByte (msg, SV_ModelIndex(GetString(ent->v.weaponmodel)));
	...
	//johnfitz -- PROTOCOL_FITZQUAKE
	if (bits & SU_WEAPON2)
		MSG_WriteByte (msg, SV_ModelIndex(GetString(ent->v.weaponmodel)) >> 8);
	if (bits & SU_ARMOR2)
...
int SV_ModelIndex (const char *name)
{
	...
	for (i=0 ; i<MAX_MODELS && sv.model_precache[i] ; i++)
                //Con_SafePrintf("SV_ModelIndex %i %s vs %s\n", i, sv.model_precache[i], name);
                //Con_SafePrintf("SV_ModelIndex %i %s\n", i, name);
...
void SV_SaveSpawnparms (void)
{
	...
	// call the progs to get default spawn parms for the new client
		...
		ExecuteProgram (pr_global_struct->SetChangeParms);
...
// leave slots at start for clients only
	...
	memset(sv.edicts, 0, sv.max_edicts*pr_edict_size); // ericw -- sv.edicts switched to use malloc()
	for (i=0 ; i<svs.maxclients ; i++)
	{
		ent = EDICT_NUM(i+1);
...
//
	...
	ent->free = false;
	ent->v.model = SetEngineString(sv.worldmodel->name);
...
*/
	...
	pr_global_struct->mapname = SetEngineString(sv.name);

// serverflags are for cross level information (sigils)
	pr_global_struct->serverflags = svs.serverflags;

	LoadFromFile (sv.worldmodel->entities);
