	...
	{	// explicit hulls in the BSP model
		if (ent->v.movetype != MOVETYPE_PUSH)
			Host_Error ("SOLID_BSP without MOVETYPE_PUSH (%s at %f %f %f)",
				    GetString(ent->v.classname), ent->v.origin[0], ent->v.origin[1], ent->v.origin[2]);
...
iteating the trigger_edicts linked list while calling ExecuteProgram
...
void SV_TouchLinks (edict_t *ent)
{
	...
	{
		touch = list[i];
	// re-validate in case of ExecuteProgram having side effects that make
