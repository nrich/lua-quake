...
void Host_Name_f (void)
{
	...
	host_client->edict->v.netname = SetEngineString(host_client->name);
...
void Host_Kill_f (void)
{
	...
	ExecuteProgram (pr_global_struct->ClientKill);
}
...
// run the entrance script
	...
	else
	{
		...
		ent->v.netname = SetEngineString(host_client->name);
		...
		pr_global_struct->time = sv.time;
		pr_global_struct->self = EDICT_TO_PROG(sv_player);
		ExecuteProgram (pr_global_struct->ClientConnect);

		if ((Sys_DoubleTime() - NET_QSocketGetTime(host_client->netconnection)) <= sv.time)
			Sys_Printf ("%s entered the game\n", host_client->name);

		ExecuteProgram (pr_global_struct->PutClientInServer);
	}
...
edict_t	*FindViewthing (void)
{
	...
	{
		e = EDICT_NUM(i);
		if ( !strcmp (GetString(e->v.classname), "viewthing") )
			return e;
	}
