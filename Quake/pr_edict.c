...
static ddef_t *ED_FindField (const char *name)
{
	...
	int			i;
        ...
        if (use_luaVM)
	    Host_Error ("ED_FindField\n");

        Con_SafePrintf("?!!!! %d\n", progs->numfielddefs);
	for (i = 0; i < progs->numfielddefs; i++)
	{
		def = &pr_fielddefs[i];
                Con_SafePrintf("%d %s\n", i, GetString(def->s_name));
...
const char *PR_GlobalStringNoContents (int ofs)
{
	...
	ddef_t		*def;
        ...
        if (use_luaVM)
	    Host_Error ("PR_GlobalStringNoContents\n");
	...
	while (1)
	{
		...
		if (!key)
		{
			...
			continue;
                ...
                Con_SafePrintf("Field is '%s' '%s' %d\n", keyname, com_token, key->ofs);
		...
		if (anglehack)
		{
			...
			sprintf (com_token, "0 %s 0", temp);
                ...
                if (use_luaVM) {
                    Con_SafePrintf("l_ParseEpair %s %s\n", keyname, com_token); 
...
//
		//if (!ent->v.classname || ent->v.classname == -1)
	...
	// look for the spawn function
		...
		ExecuteProgram (func - pr_functions);
	}
...
edict_t *EDICT_NUM(int n)
{
	if (n < 0 || n >= sv.max_edicts)
		Host_Error ("EDICT_NUM: bad number %i %i", n, sv.max_edicts);
...
int NUM_FOR_EDICT(edict_t *e)
{
	...
	if (b < 0 || b >= sv.num_edicts)
		Host_Error ("NUM_FOR_EDICT: bad pointer %d %d", b, sv.num_edicts);
	return b;
}
...
{
        if (use_luaVM)
    	    Host_Error ("PR_AllocStringSlots\n");
...
const char *PR_GetString (int num)
{
	...
	{
		Host_Error("PR_GetString: invalid string offset %d %d\n", num, -pr_numknownstrings);
		return "";
	}
}
